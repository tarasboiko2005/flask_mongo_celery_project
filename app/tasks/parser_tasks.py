...
def parse_page(job_id: str, url: str, user_email: str | None = None, limit: int = 5):
    ...
    try:
        ...
        jobs.update_one(
            {"job_id": job_id}, {"$set": {"status": "failed", "progress": 100}}
        )
        if user_email:
            send_job_report.delay(
                user_email, job_id, "failed", f"Error fetching {url}: {str(e)}"
            )
        return {"error": str(e)}
    ...
    for full_url in images:
        try:
            ...
            processed_files.append(
                ParsedImage(filename=filename, file_path=filepath, source_url=full_url)
            )
            add_metadata(
                f"Parsed image {filename} from {full_url} at {datetime.utcnow()}"
            )
    ...
    if status == "ready":
        if user_email:
            send_job_report.delay(
                ...
                job_id,
                "ready",
                f"Parsed {len(processed_files)} images from {url}",
            )
    else:
        if user_email:
            send_job_report.delay(
                user_email, job_id, "failed", f"No images could be processed from {url}"
            )
